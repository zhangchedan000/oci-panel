...
**只读**：配额查询、流量统计(近6小时)
**账号**：多账号切换；**在面板里添加/删除账号**（粘整段 [DEFAULT] 配置自动解析 + 粘贴或上传 .pem 私钥 + 分字段填代理[http/https/socks5/socks5h]，自动写盘热加载；非 HTTPS 会警告勿贴私钥）；每账号「出口 IP 设置」窗口(代理/源IP/直连，可热切换)+「检测当前出口 IP」核对绑定；每账号固定指纹(自动分配)
